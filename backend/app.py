...
def update_vehicle_admin(vehicle_id):
    token = request.headers.get('Authorization')
    if not token:
        return jsonify({'code': 401, 'message': '未提供token'}), 401
    ...
    try:
        ...
        return jsonify({'code': 200, 'message': '车辆更新成功'})
    except jwt.ExpiredSignatureError:
        return jsonify({'code': 401, 'message': 'token已过期'}), 401
    except jwt.InvalidTokenError:
        return jsonify({'code': 401, 'message': '无效的token'}), 401
...
def get_parking_lots():
    ...
    try:
        ...
        # 动态计算每个停车场的空闲车位数量
        parking_lots_with_available = []
        for lot in parking_lots:
            lot_dict = dict(lot)
            # 计算空闲车位数量
            available_spaces = conn.execute('''
                SELECT COUNT(*) FROM parking_spaces 
                WHERE parking_lot_id = ? AND status = 'free'
            ''', (lot_dict['id'],)).fetchone()[0]
            lot_dict['available_spaces'] = available_spaces
            parking_lots_with_available.append(lot_dict)
        
        conn.close()

        return jsonify({'code': 200, 'data': parking_lots_with_available})
    except jwt.ExpiredSignatureError:
        return jsonify({'code': 401, 'message': 'token已过期'}), 401
    except jwt.InvalidTokenError:
        return jsonify({'code': 401, 'message': '无效的token'}), 401
...
def get_user_reservations():
    ...
    try:
        ...
        reservations = conn.execute('''
            SELECT r.*, v.license_plate, v.brand, v.color, pl.name as parking_lot_name
            FROM reservations r
        ...
        return jsonify({'code': 200, 'data': [dict(reservation) for reservation in reservations]})
    except jwt.ExpiredSignatureError:
        return jsonify({'code': 401, 'message': 'token已过期'}), 401
    except jwt.InvalidTokenError:
        return jsonify({'code': 401, 'message': '无效的token'}), 401
...
def create_reservation():
    ...
    try:
        ...
        try:
            # 检查车位是否存在
            cursor = conn.execute('SELECT status FROM parking_spaces WHERE id = ?', (parking_space_id,))
            space_status = cursor.fetchone()
            
            if not space_status:
                conn.close()
                return jsonify({'code': 400, 'message': '该车位不存在'}), 400
            
            # 如果车位已被预约，检查是否是同一用户
            if space_status[0] == 'booked':
                # 查找该车位的现有预约
                existing_reservation = conn.execute('''
                    SELECT id FROM reservations 
                    WHERE parking_space_id = ? AND status = 'booked' AND user_id = ?
                ''', (parking_space_id, user_id)).fetchone()
                
                if existing_reservation:
                    # 取消现有预约，以便替换
                    conn.execute('UPDATE reservations SET status = ? WHERE id = ?', ('cancelled', existing_reservation[0]))
                else:
                    conn.close()
                    return jsonify({'code': 400, 'message': '该车位已被其他用户预约'}), 400
            ...
            # 更新车位状态为已预约
            conn.execute('UPDATE parking_spaces SET status = ? WHERE id = ?', ('booked', parking_space_id))
            
            # 重新计算并更新可用车位数
            free_spaces = conn.execute('''
                SELECT COUNT(*) FROM parking_spaces WHERE parking_lot_id = ? AND status = 'free'
            ''', (parking_lot_id,)).fetchone()[0]
            conn.execute('UPDATE parking_lots SET available_spaces = ? WHERE id = ?', (free_spaces, parking_lot_id))
...
def cancel_reservation(reservation_id):
    ...
    try:
        ...
        try:
            # 获取预约详情
            cursor = conn.execute('''
                SELECT r.*, p.parking_lot_id 
                FROM reservations r 
                JOIN parking_spaces p ON r.parking_space_id = p.id 
                WHERE r.id = ?
            ''', (reservation_id,))
            ...
            # 检查用户是否有权取消该预约
            if reservation[1] != user_id and role != 'admin':
                conn.close()
                return jsonify({'code': 403, 'message': '没有权限取消该预约'}), 403
            
            # 检查预约状态
            if reservation[7] in ['cancelled', 'completed']:
                conn.close()
                return jsonify({'code': 400, 'message': '该预约已被取消或完成'}), 400
            
            parking_space_id = reservation[3]
            parking_lot_id = reservation[9]
            
            # 更新预约状态为已取消
            conn.execute('UPDATE reservations SET status = ? WHERE id = ?', ('cancelled', reservation_id))
            
            # 更新车位状态为空闲
            conn.execute('UPDATE parking_spaces SET status = ? WHERE id = ?', ('free', parking_space_id))
